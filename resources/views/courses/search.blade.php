...
@section('title', 'Search - Course OBITO LMS')
@section('content')
    <x-navigation-auth />
        ...
        <section id="result" class="flex flex-col w-full max-w-[1280px] px-[75px] gap-5 mx-auto">
            ...
            @if ($searchCourses->count() > 0)
                <div id="result-list" class="tab-content grid grid-cols-4 gap-5">
                    @foreach ($searchCourses as $search)
                        <x-course-component :course="$search" />
                    @endforeach
                </div>
            @else
                <div class=" flex justify-center align-center gap-10">
                    <h1 class="text-center flex justify-center align-center text-semibold text-3xl text-red-600">No course
                        found
                    </h1>
                </div>
            @endif
